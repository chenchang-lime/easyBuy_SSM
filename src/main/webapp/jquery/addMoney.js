...
function addMoney(){
	...
	jq.post("/easyBuy_SSM/user/addMoney/"+money,function(data){
		...
		if(data.result=="ok"){
			...
			jq(".b_sure").attr("href","javascript:void(0)").attr("onclick","CloseDiv('showMsgDiv','fade')").html("转转");
			jq(".b_buy").attr("href","javascript:void(0)").attr("onclick","CloseDiv('showMsgDiv','fade')").html("留下");
			ShowDiv('showMsgDiv','fade');
		}else{
